...
import { SUPERVISE_WIDGET_TABS } from '@/components/RetrospectiveWidgetForm/retrospectiveWidgetTabs';
import { TabPanel } from '@/components/TabPanel/TabPanel';
import { useTabs } from '@/lib/utils/hooks/useTabs';
import { IRetrospectiveDetailsContentProps } from './types';

export const SuperviseDetailsWidget: FC<IRetrospectiveDetailsContentProps> =
  memo(() => {
    ...
    return (
      ...
      >
        <Tabs value={activeTab} onChange={changeTab}>
          {SUPERVISE_WIDGET_TABS.map(({ label }, i) => (
            <Tab sx={{ flex: 1 }} key={label} label={t(label)} value={i} />
          ))}
        </Tabs>
        {SUPERVISE_WIDGET_TABS.map(({ content, label }, i) => (
